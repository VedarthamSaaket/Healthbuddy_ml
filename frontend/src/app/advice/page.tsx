...
const UI: Record<string, Record<string, string>> = {
  ...
  te: {
    ...
    regenerating: 'తిరిగి రూపొందిస్తున్నారు',
  ...
  kn: {
    ...
    regenerating: 'ಮರು-ರಚಿಸಲಾಗುತ್ತಿದೆ',
...
function SectionBlock({ title, content }: { title: string; content: string }) {
  ...
  return (
    <div style={{ marginBottom: '24px' }}>
      ...
      <ul style={{ margin: 0, paddingLeft: '30px', listStyle: 'none' }}>
        {lines.map((line, i) => {
          const cleaned = line.replace(/^\*\*[^*]+\*\*\s*/, '').replace(/^[-•·]\s*/, '').trim()
          if (!cleaned) return null
          return (
            <li key={i} style={{ display: 'flex', gap: '10px', marginBottom: '6px', alignItems: 'flex-start' }}>
              <span style={{ color: 'rgba(168,192,232,0.35)', fontSize: '0.6rem', marginTop: '6px', flexShrink: 0 }}>◆</span>
              <span style={{ fontFamily: "'Crimson Pro', serif", fontSize: '1.02rem', lineHeight: 1.7, color: 'rgba(255,255,255,0.72)' }}>
                {cleaned}
              </span>
            </li>
          )
...
export default function MedicationPage() {
  ...
  const [resultLang, setResultLang] = useState<string>('en')
  ...
  const generateForLang = async (targetLang: string) => {
    ...
    if (isInitial) {
      ...
      setRegeneratingLang(targetLang)
    }
    setResult(null)
    try {
      const data = await fetchJSON(`${API_BASE}/api/medication/generate`, {
        ...
        body: JSON.stringify({ query: query.trim(), language: targetLang, symptom_log_id: selectedLogId }),
      })
      setResult(data)
  ...
  const generate = () => generateForLang(lang)
              ...
              <motion.div key="generate" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}>
                ...
                {loadingData ? (
                  ...
                  <>
                    ...
                    {/* Generate button */}
                    <motion.button onClick={generate} disabled={loading || !!regeneratingLang} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
                      style={{
                        width: '100%', padding: '16px', borderRadius: '4px',
                        border: '1px solid rgba(168,192,232,0.35)',
                        background: (loading || !!regeneratingLang) ? 'rgba(30,58,138,0.15)' : 'rgba(30,58,138,0.25)',
                        fontFamily: "'DM Sans', sans-serif", fontSize: '0.68rem', letterSpacing: '3px', textTransform: 'uppercase',
                        color: (loading || !!regeneratingLang) ? 'rgba(168,192,232,0.5)' : 'rgba(168,192,232,0.9)',
                        cursor: (loading || !!regeneratingLang) ? 'not-allowed' : 'pointer', marginBottom: '40px',
                      ...
                      {result && (
                        ...
                        >
                          ...
                          <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '6px' }}>
                            ...
                            {otherLangs.map(l => {
                              const isRegen = regeneratingLang === l.code
                              return (
                                <motion.button
                                  key={l.code}
                                  onClick={() => !regeneratingLang && generateForLang(l.code)}
                                  disabled={!!regeneratingLang}
                                  whileHover={{ scale: regeneratingLang ? 1 : 1.06 }}
                                  whileTap={{ scale: regeneratingLang ? 1 : 0.95 }}
                                  style={{
                                    padding: '6px 14px', borderRadius: '3px',
                                    cursor: regeneratingLang ? 'not-allowed' : 'pointer',
                                    border: isRegen ? '1px solid rgba(201,169,110,0.5)' : '1px solid rgba(201,169,110,0.25)',
                                    background: isRegen ? 'rgba(201,169,110,0.12)' : 'transparent',
                                    fontFamily: "'DM Sans', sans-serif", fontSize: '0.58rem', letterSpacing: '1.5px',
                                    color: isRegen ? '#c9a96e' : 'rgba(201,169,110,0.6)',
                                ...
                                >
                                  {isRegen && (
                                    <motion.div
                                      style={{ width: '8px', height: '8px', borderRadius: '50%', border: '1.5px solid rgba(201,169,110,0.3)', borderTopColor: '#c9a96e', flexShrink: 0 }}
                                      animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}
                                    />
                                  )}
                                  {l.short}
                            ...
                            {resultLang !== lang && (
                              <motion.button
                                onClick={() => !regeneratingLang && generateForLang(lang)}
                                disabled={!!regeneratingLang}
                                whileHover={{ scale: regeneratingLang ? 1 : 1.04 }}
                                style={{
                                  padding: '6px 14px', borderRadius: '3px',
                                  cursor: regeneratingLang ? 'not-allowed' : 'pointer',
                                ...
                                title="Back to page language"
                              >
                                ↩ {LANG_OPTIONS.find(l2 => l2.code === lang)?.short}
                              </motion.button>
                            )}
                          </div>

                          {/* Recommendations display */}
                          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(90,127,196,0.25)', borderRadius: '4px', padding: '32px', position: 'relative' }}>
                            {regeneratingLang && (
                              <div style={{ position: 'absolute', inset: 0, background: 'rgba(13,24,41,0.7)', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2, gap: '12px' }}>
                                <motion.div
                                  style={{ width: '16px', height: '16px', borderRadius: '50%', border: '2px solid rgba(168,192,232,0.2)', borderTopColor: '#a8c0e8' }}
                                  animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}
                                />
                                <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.6rem', letterSpacing: '2px', textTransform: 'uppercase', color: 'rgba(168,192,232,0.6)' }}>
                                  {t.regenerating} {LANG_OPTIONS.find(l => l.code === regeneratingLang)?.nativeLabel}…
                                </span>
                              </div>
                            )}
                            {parseRecommendations(result.recommendations).map((section, i, arr) => (
                              <motion.div key={`${resultLang}-${i}`} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}>
